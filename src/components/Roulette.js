...
class Roulette extends Component {
  ...
  drawRouletteWheel() {
    ...
    if (canvas.getContext) {
      ...
      const textRadius = baseSize - 75;
      const insideRadius = baseSize - 115;
  ...
  stopRotateWheel() {
    ...
    ctx.fillText(text, baseSize - ctx.measureText(text).width / 2, baseSize / 1);
    ctx.restore();
    ...
    return (
      <div className="roulette">
        <div className="roulette-container">
          <input type="button" value="GIRAR LA RULETA" onClick={this.handleOnClick} className="button" id="spin" />
        </div>
        <div className="roulette-container">
